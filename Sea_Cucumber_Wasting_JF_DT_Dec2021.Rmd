...
# temperature data.
tempRange <- DailyLog %>%
  filter(date == "2021-11-11" | date == "2021-11-12" | date =="2021-11-10") %>%
...
Functions that generate data frames with death_time, spawning, evisceration, and poop data. 
...
We read in longitudinal data frames with behaviours measured over time, as this data will later be used for stiffness analysis
```{r,, warning=FALSE, message=FALSE}
...
We use our data frames generated above to conduct analyses on our data. The results of statistical tests and analyses are commented out below the approriate line of code in each function. 

The following statistical packages are required for our analyses:
```{r, warning=FALSE, message=FALSE}
library(gamlss)
library(FSA)
library(ordinal)
...
Done to make sure that temps were different in our treatments (statistically confirmed with a K-W test).
```{r, warning=FALSE}
...
# testing for normality, data is NOT normal
shapiro.test(tempRange$temp_C)

# differences are highly significant 
kruskal.test(temp_C ~treatment, data = tempRange)
# p < 2.2e-16
...
First, we examine the correlation between the two stiffness scores with the rcorr() function. Then, we use clmm() from the `ordinal` package to analyze data. We use dredge() from the MuMIn package to do model selection (code not run because of long processing times)

Note that Droop = stiffness for posture maintenance, Squeeze = stiffness for antipredator defense.
...
To look at per-bucket spawning we use KW tests and observed no differences.
```{r, warning=FALSE}
...
We model the effect of treatment, weight, and defecation status, along with random effects, on evisceration.
```{r, warning=FALSE}
...
Figures 1 and 2, which are multipanel figures of images, were made in Microsoft Powerpoint. The powerpoint file and individual images are included in our git repository.
...
## Figure 4, 5 - Stiffness

Figure 4 = antipredator stiffness
Figure 5 = posture stiffness
...
droop_plot =
  ...
  geom_vline(xintercept=1.5, linetype=2, size=1)+ geom_vline(xintercept=4.5, linetype=2, size=1)+
  scale_y_continuous(expand=c(0,0))+
  scale_fill_manual(name="Postural Stiffness",labels=c("0 - Soft","1 - Standard","2 - Stiff"), 
...
## Figure S2 : Correlation between antipredator and Postural stiffness
```{r, warning=FALSE}
...
squeeze_droop_cor = ggplot(data=stiff, aes(x=Squeeze_score, y=Droop_score, color=Treatment))+
  ...
  xlab("Antipredator Defense") + ylab("Postural Maintenance")+
  theme(strip.text.y = element_text(size =12),
        legend.position="NONE")+
  facet_grid(Treatment~., labeller=labeller(Treatment = facet.labs))
...
droop_output = data.frame(coef = coef(Droop_Top), # coef
                          ...
                          stiff = "Postural Maintenance")[-c(1,2),]
