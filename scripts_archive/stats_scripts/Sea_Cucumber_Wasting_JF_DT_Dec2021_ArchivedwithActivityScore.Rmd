...
## Activity
Use a series of kruskal wallis tests (followed up with a Dunn test when differences are significant) 
We then used a zero adjusted negative binomial regression to model differences in activity score 
```{r, warning=FALSE}
...
# Day 12 (after recovery) - p = 0.4
kruskal.test(Activity_Score ~ Treatment, data=subset(Act, Date=="2021-11-20"))

# running Dunn tests to compare treatments on 11-09, which was significant
FSA::dunnTest(Activity_Score ~ Treatment, data=subset(Act, Date=="2021-11-09")) 
...
# zero adjusted negative binomial distribution is the best
...
step.Act.backward <- stepGAIC(Act_Full, direction = "backward", trace = F)
...
ggsave("Fig4_temp_plot.png", TempPlot, device = "png", path = here("figures"), width=8, height=5)
```

## Figure 5 & 6 Stiffness
Figure 5 = antipredator stiffness
Figure 6 = structural stiffness
...
ggsave("figures/Fig5_antipredator_stiffness.jpg",plot=squeeze_plot, width=6, height=5)
...
ggsave("figures/Fig6_structural_stiffness.jpg",plot=droop_plot, width=6, height=5)


```


## Figure 7: Activity Scores
```{r, warning=FALSE}
Activity_plot = 
...
ggsave("figures/Fig7_activity.jpg",plot=Activity_plot, width=6, height=4)
```
